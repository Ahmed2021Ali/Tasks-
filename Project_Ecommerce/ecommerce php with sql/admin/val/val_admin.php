...
if(CheckRequestMethod("POST") && isset($_POST['name'])  && isset($_POST['Address'])  && isset($_POST['email']) && isset($_POST['password'])  )
{
    ...
    $sql="SELECT * FROM `user` WHERE `email` LIKE '$email' AND `status` LIKE 'admin' ";
    ...
    if (empty($errors))
    {
        ...
        $connect= connectdatabase();
        $sql="INSERT INTO `user` (`name`,`email`,`password`,`Address`,`status`) VALUES ('$name','$email','$password','$Address','admin')";
        $Result=mysqli_query($connect,$sql);
    }
    else
    {
